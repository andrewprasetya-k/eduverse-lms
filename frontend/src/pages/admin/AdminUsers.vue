...
import type { AdminUserItem, RoleItem, SchoolMemberItem } from "../../types/adminUser";
...
const allowedRoles = computed(() =>
  roles.value.filter((role) => allowedRoleNames.includes(normalizeRoleName(role.roleName))),
);
...
function initializeRoleDrafts() {
  const roleByName = new Map(
    allowedRoles.value.map((role) => [normalizeRoleName(role.roleName), role.roleId]),
  );
  ...
  for (const member of members.value) {
    ...
    nextDrafts[member.schoolUserId] = ids;
  }
  ...
  try {
    ...
    if (!existingRoleId.value) {
      existingRoleId.value =
        allowedRoles.value.find((role) => normalizeRoleName(role.roleName) === "student")?.roleId ?? "";
    }
  } catch {
...
async function addExistingUser(user: AdminUserItem) {
  ...
  try {
    ...
    if (!member) {
      actionError.value = "Membership belum ditemukan setelah user ditambahkan.";
      return;
    }

    await syncUserRoles(member.schoolUserId, { roleIds: [existingRoleId.value] });
...
function isRoleChecked(schoolUserId: string, roleId: string) {
  return memberRoleDrafts.value[schoolUserId]?.includes(roleId) ?? false;
}
...
function toggleRoleDraft(schoolUserId: string, roleId: string) {
  ...
  memberRoleDrafts.value = {
    ...memberRoleDrafts.value,
    [schoolUserId]: current.includes(roleId)
    ...
    <section class="mx-auto flex max-w-6xl flex-col gap-6">
      <header class="soft-card rounded-[22px] p-5">
        <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div>
            ...
            <h1 class="mt-2 text-2xl font-medium text-[#111827]">User, membership, dan role</h1>
            <p class="mt-2 max-w-3xl text-sm leading-6 text-[#6B7280]">
              Kelola membership sekolah dari akun global yang sudah ada, lalu beri role dalam konteks
              sekolah aktif sebelum enrollment kelas.
            </p>
          </div>
      ...
      <section class="grid gap-5">
        <article class="bg-white border border-[#EBEBEB] rounded-[18px] p-5">
          <div class="flex items-start justify-between gap-4">
            <div>
              <p class="text-[11px] font-medium uppercase text-[#9CA3AF]">Existing global account</p>
              <h2 class="mt-2 text-base font-medium text-[#111827]">Tambah user yang sudah ada</h2>
              <p class="mt-1 max-w-2xl text-sm leading-6 text-[#6B7280]">
                Akun user bersifat global. School admin hanya menghubungkan akun yang sudah ada
                sebagai membership sekolah aktif, lalu memberi role sekolah.
              </p>
            </div>
            <PhMagnifyingGlass :size="22" class="text-[#4F46E5]" weight="duotone" />
          </div>

          <form class="mt-5 flex flex-col gap-3 sm:flex-row" @submit.prevent="searchUsers">
            <input
              v-model="userSearch"
              type="search"
          ...
          <label class="mt-4 block text-sm font-medium text-[#374151]">
            ...
            >
              ...
              <option v-for="role in allowedRoles" :key="role.roleId" :value="role.roleId">
          ...
          <div class="mt-5 space-y-3">
            <p v-if="userSearchLoading" class="text-sm text-[#6B7280]">Mencari akun global...</p>
            <p v-else-if="userSearchError" class="text-sm text-[#DC2626]">{{ userSearchError }}</p>
            <p v-else-if="userResults.length === 0" class="text-sm text-[#6B7280]">
              Cari akun global yang sudah ada untuk ditambahkan ke sekolah aktif.
            </p>
            ...
            >
              <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  <h3 class="text-sm font-medium text-[#111827]">{{ user.fullName }}</h3>
                ...
                <button
                  ...
                  :disabled="isMember(user.userId) || !existingRoleId || addExistingLoadingUserId === user.userId"
      ...
      <section class="bg-white border border-[#EBEBEB] rounded-[18px] p-5">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div>
            <p class="text-[11px] font-medium uppercase text-[#9CA3AF]">School members</p>
            <h2 class="mt-2 text-base font-medium text-[#111827]">Member sekolah aktif</h2>
            <p class="mt-1 text-sm text-[#6B7280]">
              Role disimpan pada school_user, bukan pada user global.
            </p>
          </div>
        ...
        <form class="mt-5 flex flex-col gap-3 sm:flex-row" @submit.prevent="loadMembers">
          <input
        ...
        <div class="mt-5">
          <div v-if="rolesLoading" class="rounded-[18px] bg-[#FBFAF8] p-5 text-sm text-[#6B7280]">
            Memuat role...
          </div>
          <div v-else-if="rolesError" class="rounded-[18px] border border-[#FECACA] bg-[#FEF2F2] p-5 text-sm text-[#DC2626]">
            {{ rolesError }}
          </div>
            ...
            >
              <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                <div>
                  ...
                  <p class="mt-1 text-xs text-[#6B7280]">{{ member.email || "Email tidak tersedia" }}</p>
                ...
                <div class="w-full max-w-xl">
                  <div class="flex flex-wrap gap-2">
                    <label
                      ...
                      :key="role.roleId"
                      class="inline-flex items-center gap-2 rounded-lg border border-[#EBEBEB] bg-white px-3 py-2 text-xs font-medium text-[#374151]"
                    >
                      <input
                        type="checkbox"
                        class="h-3.5 w-3.5 rounded border-[#D1D5DB] text-[#4F46E5]"
                        :checked="isRoleChecked(member.schoolUserId, role.roleId)"
                  ...
                  <button
                    ...
                    :disabled="savingRolesSchoolUserId === member.schoolUserId || (memberRoleDrafts[member.schoolUserId]?.length ?? 0) === 0"
                    @click="syncRoleForMember(member.schoolUserId, memberRoleDrafts[member.schoolUserId] ?? [])"
                  >
