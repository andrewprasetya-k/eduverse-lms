...
const materialProgress = computed(() => {
  ...
  return Math.round((summary.value.completedMaterials / summary.value.totalMaterials) * 100)
})
...
async function loadDashboard() {
  ...
  try {
    const schoolUserId = auth.defaultContext?.schoolUserId ?? activeMembership.value?.schoolUserId
    const [dashboardData, notificationData, unreadData, classData] = await Promise.all([
...
function nearestDeadlineFor(subjectName?: string) {
  ...
  return deadlines.find((deadline) => deadline.subjectName === subjectName) ?? deadlines[0]
}
...
function buildCalendarDays(date: Date) {
  ...
  for (let i = 0; i < startOffset; i += 1) {
    days.push({ key: `empty-${i}`, label: '', isToday: false, hasEvent: false })
  }
...
<template>
  <main class="grid min-h-screen flex-1 grid-cols-1 overflow-hidden lg:grid-cols-[1fr_320px]">
    <section class="flex flex-col gap-6 px-5 py-6 sm:px-8 lg:px-10">
      ...
      <div v-if="errorMessage" class="soft-card rounded-[22px] p-5 text-sm text-[#b42318]">
        {{ errorMessage }}
      </div>
          ...
          <div class="space-y-3 px-4 py-4">
            <p class="min-h-9 text-xs leading-5 text-[#7a7385]">
              <template v-if="nearestDeadlineFor(item.subjectName)">
                ...
                <span> - {{ nearestDeadlineFor(item.subjectName)?.assignmentTitle }}</span>
              </template>
              <template v-else>
            ...
            <div class="flex items-center gap-2">
              ...
              <span class="w-8 text-right text-[11px] text-[#9a95a3]">{{ materialProgress }}%</span>
            </div>
          </div>
        </article>
      </section>
      ...
      <section class="grid gap-4 xl:grid-cols-[1.1fr_0.9fr]">
        <article class="soft-card rounded-[22px] p-5">
          <div class="mb-4 flex items-center justify-between">
            <div>
              <p class="text-sm font-medium text-[#171322]">Aktivitas akademik</p>
              <p class="text-xs text-[#8b8592]">Diambil dari dashboard siswa backend</p>
            </div>
          ...
          <div class="grid gap-3 sm:grid-cols-3">
            ...
            <div class="rounded-2xl bg-[#f3ecff] p-4">
              <p class="text-2xl font-medium text-[#9d5bd2]">{{ materialProgress }}%</p>
        ...
        <article class="soft-card rounded-[22px] p-5">
          ...
          <div v-if="summary?.upcomingDeadlines?.length" class="mt-4 space-y-3 text-sm text-[#6b6475]">
            <div
              v-for="deadline in summary.upcomingDeadlines.slice(0, 2)"
              :key="deadline.assignmentId"
              class="rounded-2xl bg-[#fbfaf8] p-3"
            >
              <p class="font-medium text-[#3f3a4a]">{{ deadline.subjectName }}</p>
              <p class="mt-1 text-xs">{{ deadline.assignmentTitle }} - {{ deadline.deadline }}</p>
            </div>
          </div>
          <p v-else class="mt-4 rounded-2xl bg-[#fbfaf8] p-3 text-sm text-[#7a7385]">
    ...
    <aside class="border-l border-[#ebe7df] bg-white/95">
      ...
      <div v-if="isLoading" class="space-y-2 p-4">
        <div v-for="item in 4" :key="item" class="h-16 animate-pulse rounded-2xl bg-[#f8f7f4]" />
      </div>
        ...
        >
          ...
          <div class="min-w-0 flex-1">
            <div class="flex items-baseline justify-between gap-2">
              <p class="truncate text-sm font-medium text-[#171322]">{{ item.title }}</p>
