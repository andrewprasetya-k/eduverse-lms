...
import type { GradebookAssignment, MyGradebookResponse } from '../../types/studentGrades'
...
<template>
  <main class="min-h-screen flex-1 bg-[#f8f7f4]">
    <section class="border-b border-[#ebe7df] bg-white px-5 py-3 sm:px-8 lg:px-10">
      <div class="flex flex-wrap items-center justify-between gap-3">
        <div class="flex items-center gap-2">
          <span class="text-xs text-[#9a95a3]">Kelas aktif:</span>
          <div class="flex items-center gap-2 rounded-xl border border-[#ebe7df] bg-[#f9fafb] px-3 py-2">
            <div class="flex h-5 w-5 items-center justify-center rounded-md bg-[#4f46e5] text-[10px] text-white">
              {{ activeClass?.classTitle?.slice(0, 2).toUpperCase() || 'EV' }}
            </div>
            <div>
              <p class="text-xs font-medium text-[#171322]">
                {{ activeClass?.classTitle || gradebook?.class.className || 'Belum ada kelas aktif' }}
              </p>
          ...
          <select
            ...
            @change="changeActiveClass(($event.target as HTMLSelectElement).value)"
          >
            <option
    ...
    <section class="px-5 py-6 sm:px-8 lg:px-10">
      <header class="mb-6 flex flex-col gap-2">
        ...
        <h1 class="text-2xl font-medium tracking-normal text-[#171322]">Nilai dan feedback</h1>
        <p class="max-w-2xl text-sm leading-6 text-[#7a7385]">
          Nilai dikelompokkan berdasarkan subject pada kelas aktif. Data hanya menampilkan
          pengumpulan dan penilaian milik akun login saat ini.
        </p>
      </header>
      ...
      >
        <div class="h-40 animate-pulse rounded-[22px] border border-[#ebe7df] bg-white" />
        <div class="h-40 animate-pulse rounded-[22px] border border-[#ebe7df] bg-white" />
      </section>
      ...
      <section v-else-if="errorMessage" class="soft-card max-w-2xl rounded-3xl p-6">
        ...
        <p class="text-sm font-medium text-[#171322]">Tidak bisa memuat nilai</p>
      ...
      <section v-else-if="!activeClassStore.activeClassId" class="soft-card max-w-2xl rounded-3xl p-6">
        ...
        <p class="mt-2 text-sm leading-6 text-[#7a7385]">
          Nilai akan tampil setelah akunmu terdaftar pada kelas di sekolah aktif.
        </p>
      </section>

      <section v-else-if="subjects.length === 0" class="soft-card max-w-2xl rounded-3xl p-6">
        <div class="mb-4 flex h-11 w-11 items-center justify-center rounded-2xl bg-[#eef2ff] text-[#4f46e5]">
          <PhChartBar :size="24" weight="duotone" />
        </div>
        <p class="text-sm font-medium text-[#171322]">Gradebook belum tersedia</p>
        <p class="mt-2 text-sm leading-6 text-[#7a7385]">
      ...
      <section v-else class="space-y-5">
        ...
        <div v-if="!hasAssignments" class="soft-card rounded-3xl p-6">
          ...
          <p class="text-sm font-medium text-[#171322]">Belum ada tugas untuk dinilai</p>
          <p class="mt-2 text-sm leading-6 text-[#7a7385]">
            Subject sudah tersedia, tetapi belum ada assignment pada kelas aktif.
          </p>
        </div>
        ...
        <article
          ...
          class="overflow-hidden rounded-[24px] border border-[#ebe7df] bg-white"
        >
          <div
            class="flex flex-col gap-4 px-5 py-5 text-white md:flex-row md:items-center md:justify-between"
            :style="{
              backgroundColor: getSubjectColor(
                subject.subjectClassId || subject.subjectName || subject.subjectCode,
              ),
            }"
          >
            <div>
              <p class="text-sm text-white/80">{{ subject.subjectCode || 'Kode belum tersedia' }}</p>
          ...
          <div class="grid gap-2 border-b border-[#f3f1ec] px-5 py-4 sm:grid-cols-3">
            <span class="rounded-full bg-[#ecfdf3] px-3 py-1.5 text-xs text-[#027a48]">
              {{ subject.gradedCount }} sudah dinilai
            </span>
            <span class="rounded-full bg-[#eef2ff] px-3 py-1.5 text-xs text-[#4f46e5]">
              {{ subject.submittedCount }} sudah dikumpulkan
            </span>
            <span class="rounded-full bg-[#fff7ed] px-3 py-1.5 text-xs text-[#b45309]">
              {{ subject.pendingCount }} menunggu nilai
            </span>
          </div>

          <div v-if="subject.assignments.length === 0" class="px-5 py-5">
            <p class="text-sm font-medium text-[#171322]">Belum ada assignment</p>
            <p class="mt-2 text-sm leading-6 text-[#7a7385]">
              Nilai akan muncul setelah guru membuat dan menilai tugas pada subject ini.
            </p>
          </div>
          ...
          <div v-else class="divide-y divide-[#f3f1ec]">
            ...
            >
              <div class="min-w-0">
                ...
                <div class="mt-3 grid gap-2 text-xs text-[#7a7385] sm:grid-cols-2">
                  <span class="inline-flex items-center gap-1.5">
                    <PhClock :size="14" />
                    Deadline: {{ formatDateTime(assignment.deadline) }}
                  </span>
                  <span v-if="assignment.submittedAt" class="inline-flex items-center gap-1.5">
                    <PhCheckCircle :size="14" />
                    Dikumpulkan: {{ formatDateTime(assignment.submittedAt) }}
                  </span>
                  <span v-if="assignment.assessedAt" class="inline-flex items-center gap-1.5">
                    <PhSealCheck :size="14" />
              ...
              <div class="rounded-2xl bg-[#fbfaf8] p-4">
                ...
                <p v-if="assignment.assessorName" class="mt-2 text-xs text-[#7a7385]">
