...
const palette = ['#4f8ef7', '#f2756a', '#c673d8', '#f0a05a', '#34d399', '#4f46e5']
const classes = ref<StudentClassEnrollment[]>([])
const subjects = ref<SubjectClassItem[]>([])
const activeClass = ref<StudentClassEnrollment | null>(null)
...
async function loadSubjects() {
  ...
  try {
    ...
    activeClass.value = classes.value[0] ?? null
    ...
    const data = await getSubjectClassesByClass(activeClass.value.classId)
    subjects.value = data.subjects
  } catch {
    errorMessage.value = 'Mata pelajaran belum bisa dimuat. Periksa koneksi atau coba lagi nanti.'
  } finally {
    isLoading.value = false
  }
}
  ...
  <main class="min-h-screen flex-1 bg-[#f8f7f4]">
    ...
    <section class="px-5 py-6 sm:px-8 lg:px-10">
      <header class="mb-6 flex items-center justify-between gap-4">
        <div>
          <h1 class="text-2xl font-medium tracking-normal text-[#171322]">Mata pelajaran</h1>
          <p class="mt-1 text-sm text-[#7a7385]">
            {{ subjects.length }} subject class tersedia
            <span v-if="activeClass?.classTitle"> · {{ activeClass.classTitle }}</span>
          </p>
        </div>
      </header>
      ...
      <section v-else-if="errorMessage" class="soft-card max-w-2xl rounded-3xl p-6">
        ...
        <p class="text-sm font-medium text-[#171322]">Tidak bisa memuat mata pelajaran</p>
      ...
      <section v-else-if="!activeClass" class="soft-card max-w-2xl rounded-3xl p-6">
        ...
        <p class="mt-2 text-sm leading-6 text-[#7a7385]">
          Mata pelajaran akan tampil setelah akunmu terdaftar pada kelas di sekolah aktif.
        </p>
      </section>
      ...
      <section v-else-if="subjects.length === 0" class="soft-card max-w-2xl rounded-3xl p-6">
        ...
        <p class="text-sm font-medium text-[#171322]">Subject class belum tersedia</p>
        <p class="mt-2 text-sm leading-6 text-[#7a7385]">
      ...
      <section v-else class="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
        ...
        >
          <button class="block w-full text-left" type="button" @click="openSubject(subject)">
            <div
            ...
            <div class="space-y-3 px-4 py-4">
              ...
              <div class="flex flex-wrap gap-2 border-t border-[#f3f1ec] pt-3">
                <span class="rounded-full bg-[#f3f1ec] px-2 py-1 text-[11px] text-[#6b6475]">
                  Materi
                </span>
                <span class="rounded-full bg-[#fff7ed] px-2 py-1 text-[11px] text-[#b45309]">
                  Tugas
                </span>
                <span class="rounded-full bg-[#eef2ff] px-2 py-1 text-[11px] text-[#4f46e5]">
                  Catatan nanti
                </span>
              </div>
            </div>
        ...
        <div class="mt-3 grid gap-2 sm:grid-cols-2 xl:grid-cols-3">
          ...
          >
            ...
            <div class="min-w-0 flex-1">
              ...
              <p class="text-[11px] text-[#7a7385]">Context switcher UI belum diimplementasikan</p>
            </div>
