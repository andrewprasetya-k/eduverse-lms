...
const activeMembership = computed<MembershipInfo | undefined>(() => {
  ...
  const defaultMembership = auth.memberships.find((membership) => membership.isDefault)
  return activeSchoolMembership ?? defaultMembership ?? auth.memberships[0]
})

const schoolUserId = computed(() => {
  return activeMembership.value?.schoolUserId ?? auth.defaultContext?.schoolUserId ?? ''
})

const activeSchoolName = computed(() => activeMembership.value?.school.name ?? 'Sekolah aktif')
...
async function loadDashboard() {
  ...
  try {
    ...
    errorMessage.value = 'Dashboard guru belum bisa dimuat. Coba lagi beberapa saat.'
  } finally {
    loading.value = false
  }
}
...
<template>
  <main class="min-h-screen flex-1 px-5 py-8 md:px-8 lg:px-10">
    <section class="mx-auto flex max-w-7xl flex-col gap-6">
      <div
        class="rounded-[32px] bg-[#f0e9dd] px-6 py-7 shadow-sm ring-1 ring-black/5 md:px-8"
      >
        <div class="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <p class="text-sm font-medium text-[#8a6d3b]">{{ activeSchoolName }}</p>
            ...
            <p class="mt-3 max-w-2xl text-sm leading-6 text-[#6b6475]">
              Ruang guru berfokus pada subject class yang diampu. Data di bawah memakai
              aggregate dashboard backend yang tersedia saat ini.
            </p>
          </div>
      ...
      <div v-if="!schoolUserId" class="rounded-[28px] bg-white p-6 shadow-sm ring-1 ring-black/5">
        <div class="flex items-start gap-3">
          <PhWarningCircle :size="24" class="mt-0.5 text-[#e58f86]" weight="duotone" />
          <div>
            <h2 class="text-lg font-medium text-[#171322]">Konteks guru belum tersedia</h2>
            <p class="mt-2 text-sm leading-6 text-[#6b6475]">
              Dashboard guru membutuhkan schoolUserId dari membership login. Pastikan akun memiliki
              role teacher pada school aktif.
            </p>
          </div>
        </div>
      </div>
      ...
      <template v-else>
        <div class="grid gap-4 md:grid-cols-3">
          ...
          >
            ...
            <p class="mt-2 text-sm leading-5 text-[#8a8494]">{{ item.helper }}</p>
          </article>
        </div>

        <div v-if="loading" class="rounded-[28px] bg-white p-6 shadow-sm ring-1 ring-black/5">
          <p class="text-sm text-[#6b6475]">Memuat data dashboard guru...</p>
        </div>

        <div v-else-if="errorMessage" class="rounded-[28px] bg-white p-6 shadow-sm ring-1 ring-black/5">
          <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <h2 class="text-lg font-medium text-[#171322]">Dashboard gagal dimuat</h2>
        ...
        <div class="grid gap-6 xl:grid-cols-[1.35fr_0.75fr]">
          <section class="rounded-[32px] bg-white p-6 shadow-sm ring-1 ring-black/5">
            <div class="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
              <div>
                <p class="text-sm font-medium text-[#7b61a8]">Subject yang diajar</p>
                <h2 class="mt-2 text-2xl font-medium text-[#171322]">Ringkasan kelas dan subject</h2>
              </div>
              <RouterLink
                to="/teacher/subjects"
            ...
            >
              ...
              >
                <div
                  ...
                  :style="{ backgroundColor: getSubjectColor(`${item.classId}-${item.subjectName}`) }"
                >
                  <PhBookOpen :size="24" weight="duotone" />
                </div>
                <p class="text-sm font-medium text-[#171322]">{{ item.subjectName }}</p>
                <h3 class="mt-1 text-xl font-medium text-[#2f2b3a]">{{ item.className }}</h3>
                <dl class="mt-5 grid grid-cols-3 gap-3 text-sm">
                  <div>
                    <dt class="text-[#8a8494]">Siswa</dt>
                    <dd class="mt-1 font-medium text-[#171322]">{{ item.totalStudents }}</dd>
                  </div>
                  <div>
          ...
          <aside class="flex flex-col gap-4">
            <article class="rounded-[28px] bg-white p-5 shadow-sm ring-1 ring-black/5">
              <div class="flex items-center gap-3">
                ...
                <div>
                  <p class="text-sm font-medium text-[#171322]">Pending submissions</p>
                  <p class="text-sm text-[#7b7486]">Review detail akan tersedia di halaman submissions.</p>
                </div>
              </div>
            </article>
              ...
              <div class="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p class="text-sm text-[#7b7486]">
                    Pengumuman class-level akan dibuat dari konteks class yang valid.
                  </p>
                </div>
              </div>
            </article>
