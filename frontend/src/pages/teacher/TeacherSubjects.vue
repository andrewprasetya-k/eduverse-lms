...
async function loadSubjects() {
  ...
  try {
    ...
    errorMessage.value = 'Subject yang diajar belum bisa dimuat. Coba lagi beberapa saat.'
  } finally {
    loading.value = false
  }
}
...
<template>
  <main class="min-h-screen flex-1 px-5 py-8 md:px-8 lg:px-10">
    <section class="mx-auto flex max-w-7xl flex-col gap-6">
      <header class="rounded-[32px] bg-[#f0e9dd] px-6 py-7 shadow-sm ring-1 ring-black/5 md:px-8">
        <p class="text-sm font-medium text-[#8a6d3b]">Teaching workspace</p>
        <h1 class="mt-3 text-3xl font-medium text-[#171322] md:text-4xl">Subject yang diajar</h1>
        <p class="mt-3 max-w-2xl text-sm leading-6 text-[#6b6475]">
          Pilih subject class untuk mengelola materi, tugas, dan melihat status submission siswa.
          Data di halaman ini berasal dari subject class yang terhubung dengan akun guru saat ini.
        </p>
      </header>
      ...
      <section v-else-if="errorMessage" class="rounded-[28px] bg-white p-6 shadow-sm ring-1 ring-black/5">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div class="flex items-start gap-3">
            ...
            <div>
              <h2 class="text-lg font-medium text-[#171322]">Gagal memuat subject</h2>
      ...
      <section v-else-if="!hasSubjects" class="rounded-[28px] bg-white p-6 shadow-sm ring-1 ring-black/5">
        <h2 class="text-lg font-medium text-[#171322]">Belum ada subject yang diajar</h2>
        <p class="mt-2 max-w-2xl text-sm leading-6 text-[#6b6475]">
          Akun guru ini belum terhubung ke subject class pada school aktif. Admin sekolah perlu
          menambahkan subject class dan mengassign guru terlebih dahulu.
        </p>
      </section>
        ...
        >
          <div class="flex items-start justify-between gap-4">
            <div
              ...
              :style="{ backgroundColor: getSubjectColor(subject.subjectClassId) }"
            >
          ...
          <h2 class="mt-1 text-2xl font-medium text-[#171322]">{{ subject.subjectName }}</h2>
          <p v-if="subject.subjectCode || subject.classCode" class="mt-2 text-sm text-[#6b6475]">
            {{ [subject.subjectCode, subject.classCode].filter(Boolean).join(' / ') }}
          </p>
          ...
          <div class="mt-6 grid grid-cols-2 gap-3 text-sm">
            <div class="rounded-2xl bg-[#faf8f4] p-3">
              ...
              <p class="mt-1 font-medium text-[#171322]">{{ subject.studentCount }}</p>
            </div>
            <div class="rounded-2xl bg-[#faf8f4] p-3">
              <div class="flex items-center gap-2 text-[#8a8494]">
              ...
              <p class="mt-1 font-medium text-[#171322]">{{ subject.materialCount }}</p>
            </div>
            <div class="rounded-2xl bg-[#faf8f4] p-3">
              <div class="flex items-center gap-2 text-[#8a8494]">
              ...
              <p class="mt-1 font-medium text-[#171322]">{{ subject.assignmentCount }}</p>
            </div>
            <div class="rounded-2xl bg-[#faf8f4] p-3">
              <div class="flex items-center gap-2 text-[#8a8494]">
              ...
              <p class="mt-1 font-medium text-[#171322]">{{ subject.pendingSubmissions }}</p>
            </div>
          </div>
        </RouterLink>
      </section>
