...
const selectedRoomName = computed(() => selectedRoom.value?.roomName || 'Ruang sekolah')
const selectedSchoolName = computed(() => selectedRoom.value?.schoolName || 'Sekolah aktif')
const canSend = computed(() => Boolean(selectedRoom.value?.canSend) && draft.value.trim().length > 0 && !isSending.value)
...
async function bootstrapChat() {
  ...
  try {
    ...
    selectedRoom.value = rooms.value.find((item) => item.roomId === room.roomId) ?? room
...
async function loadLatestMessages() {
  ...
  threadError.value = ''
  try {
    const response = await getMessages(selectedRoom.value.roomId, { limit: 50 })
...
async function refreshMessages(options: { silent?: boolean } = {}) {
  ...
  try {
    ...
    await markSelectedRoomRead()
    await nextTick()
    if (!previousLastId || lastMessage(messages.value)?.messageId !== previousLastId) {
      scrollToBottom()
    }
  } catch (error) {
    if (!options.silent) {
...
async function refreshRooms() {
  try {
    ...
    if (selectedRoom.value) {
      selectedRoom.value =
        latestRooms.find((room) => room.roomId === selectedRoom.value?.roomId) ?? selectedRoom.value
    }
  } catch {
...
async function markSelectedRoomRead(lastReadMessageId?: string) {
  if (!selectedRoom.value) return
  const latestMessageId = lastReadMessageId ?? lastMessage(messages.value)?.messageId
  try {
    await markRoomRead(selectedRoom.value.roomId, latestMessageId ? { lastReadMessageId: latestMessageId } : {})
  } catch {
...
<template>
  <main class="min-w-0 flex-1 px-5 py-6 sm:px-6 lg:px-8">
    <section class="mx-auto flex h-full min-h-[calc(100vh-3rem)] max-w-7xl flex-col gap-5">
      <header class="rounded-2xl border border-[#ebe7df] bg-white px-5 py-4 shadow-sm sm:px-6">
        <p class="text-xs font-semibold uppercase tracking-[0.18em] text-[#8b7f74]">Chat sekolah</p>
        <div class="mt-2 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div class="min-w-0">
            <h1 class="text-2xl font-semibold text-[#2f2b3a]">Ruang Diskusi</h1>
            <p class="mt-1 max-w-2xl text-sm text-[#7a7280]">
              Percakapan sekolah aktif berbasis REST. Pesan baru muncul saat refresh atau pembaruan berkala ringan.
            </p>
          </div>
          <button
            type="button"
          ...
          >
            <PhArrowClockwise :class="['h-4 w-4', isRefreshing ? 'animate-spin' : '']" />
            Refresh
          </button>
        </div>
      </header>
      ...
      <div
        v-if="isBooting"
        class="grid min-h-[560px] gap-4 overflow-hidden rounded-2xl border border-[#ebe7df] bg-white p-4 lg:grid-cols-[320px_minmax(0,1fr)]"
      >
      ...
      <div
        ...
        class="flex min-h-[420px] flex-col items-center justify-center rounded-2xl border border-[#ebe7df] bg-white px-6 py-12 text-center shadow-sm"
      >
        <div class="flex h-12 w-12 items-center justify-center rounded-full bg-red-50 text-red-500">
          <PhWarningCircle class="h-6 w-6" />
        </div>
        <h2 class="mt-4 text-lg font-semibold text-[#2f2b3a]">Chat belum bisa dibuka</h2>
      ...
      <div
        v-else
        class="grid min-h-[620px] flex-1 overflow-hidden rounded-2xl border border-[#ebe7df] bg-white shadow-sm lg:grid-cols-[320px_minmax(0,1fr)]"
      >
        <aside class="min-w-0 border-[#ebe7df] bg-white lg:border-r">
          <div class="border-b border-[#ebe7df] px-4 py-4">
            <p class="text-sm font-semibold text-[#2f2b3a]">Ruang sekolah</p>
            <p class="mt-1 text-xs text-[#8b7f74]">Satu ruang utama untuk sekolah aktif.</p>
          </div>
          ...
          <div class="space-y-2 p-3">
            <button
              ...
              :key="room.roomId"
              type="button"
              class="flex w-full min-w-0 items-center gap-3 rounded-xl border px-3 py-3 text-left transition hover:bg-[#f8f7f4]"
              :class="
            ...
            >
              <span
                class="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-[#4f46e5] text-sm font-semibold text-white"
              >
                {{ getInitials(room.schoolName || room.roomName) }}
              </span>
              <span class="min-w-0 flex-1">
                <span class="block truncate text-sm font-semibold text-[#2f2b3a]">
                  {{ room.roomName || 'Ruang sekolah' }}
                </span>
                <span class="mt-0.5 block truncate text-xs text-[#7a7280]">
                  {{ room.lastMessage?.content || room.schoolName || 'Belum ada pesan.' }}
                </span>
              </span>
              <span class="flex shrink-0 flex-col items-end gap-1">
                <span class="text-[11px] text-[#9b948c]">{{ formatTime(room.lastMessageAt) }}</span>
                <span
            ...
            <div
              v-if="rooms.length === 0"
              class="rounded-xl border border-dashed border-[#d8d2c8] bg-[#fbfaf8] px-4 py-8 text-center"
            >
              <PhChatCircleText class="mx-auto h-8 w-8 text-[#b5aa9c]" />
              <p class="mt-3 text-sm font-semibold text-[#2f2b3a]">Ruang belum tersedia</p>
              <p class="mt-1 text-xs text-[#7a7280]">Buka ulang halaman untuk membuat ruang sekolah.</p>
            </div>
          </div>
        </aside>

        <section class="flex min-w-0 flex-col bg-[#f8f7f4]">
          <div class="flex items-center gap-3 border-b border-[#ebe7df] bg-white px-4 py-3 sm:px-5">
            <div class="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-[#4f8ef7] text-sm font-semibold text-white">
              {{ roomInitial }}
            </div>
            <div class="min-w-0 flex-1">
              <h2 class="truncate text-sm font-semibold text-[#2f2b3a]">{{ selectedRoomName }}</h2>
              <p class="truncate text-xs text-[#7a7280]">{{ selectedSchoolName }} · Ruang sekolah</p>
            </div>
          </div>

          <div ref="messagesEl" class="min-h-0 flex-1 overflow-y-auto px-4 py-4 sm:px-5">
            <div class="mx-auto flex max-w-3xl flex-col gap-3">
              ...
              >
                ...
                <div class="ml-auto h-12 w-1/2 animate-pulse rounded-2xl bg-[#dfe3ff]" />
              ...
              >
                <p class="text-sm font-semibold text-red-600">{{ threadError }}</p>
                <button
                  type="button"
              ...
              <div
                ...
                class="flex min-h-[320px] flex-col items-center justify-center rounded-2xl border border-dashed border-[#d8d2c8] bg-white px-6 text-center"
              >
                <PhChatCircleText class="h-10 w-10 text-[#b5aa9c]" />
                <h3 class="mt-4 text-base font-semibold text-[#2f2b3a]">Belum ada pesan.</h3>
              ...
              <template v-else>
                ...
                >
                  ...
                  >
                    ...
                    <p class="px-2 text-[11px] text-[#9b948c]">{{ formatDateTime(message.createdAt) }}</p>
                  </div>
                </article>
              </template>
            </div>
          ...
          <form class="border-t border-[#ebe7df] bg-white px-4 py-3 sm:px-5" @submit.prevent="submitMessage">
            <p v-if="composerError" class="mb-2 text-sm text-red-600">{{ composerError }}</p>
            ...
            <p class="mt-2 text-xs text-[#9b948c]">Enter untuk kirim, Shift+Enter untuk baris baru.</p>
          </form>
        </section>
      </div>
    </section>
